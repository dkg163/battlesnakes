...
def start():
    ...
    # Print the full response details from http://127.0.0.1:8000
    try:
        response = requests.get('http://127.0.0.1:8000')
        print('Response from http://127.0.0.1:8000:')
        print('Status Code:', response.status_code)
        print('Headers:', response.headers)
        print('Content:', response.text)
    except Exception as e:
        print('Error retrieving response from http://127.0.0.1:8000:', str(e))
...
def run_server(handlers: typing.Dict):
    ...
    # @app.get("/")
    @bottle.post('/info')
    #def on_info(): DKG commented out.
    def info():
        return handlers["info"]()


#START COMMENTING OUT DKG
